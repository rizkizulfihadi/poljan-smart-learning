    ...
    return (
        <div className={"p-6 border-b border-grey border-l-black " + ( !seen ? "border-l-2" : "" )}>
            <div className="flex gap-5 mb-3">
                <img src={profile_img} className="w-14 h-14 flex-none rounded-full" alt="profile_image" />
                <div className="w-full">
                    <h1 className="font-medium text-xl text-dark-grey">
                        <span className="lg:inline-block hidden capitalize"></span>
                        <Link to={`/user/${username}`} className="mx-1 text-black underline">@{username} </Link>
                        <span className="font-normal">
                        {
                            type === 'like' ? "Menyukai artikel anda" :
                            type === 'comment' ? "Berkomentar pada" :
                            type === 'reply' ? "Membalas pada" :
                            type === 'suspend' ? "Menangguhkan artikel anda" :
                            type === 'unsuspend' ? "Membuka tangguhan artikel anda" :
                            ""
                        }
                        </span>
                    </h1>
...

            {
                ...
                <div className="ml-20 p-5 bg-grey mt-5 rounded-md">
                    <div className="flex gap-3 mb-3">
                        <img src={author_profile_img} alt="author profile img" className="w-8 h-8 rounded-full"/>
                        <div>
                            <h1>
                                <h1 className="font-medium text-xl text-dark-grey">
