        ...
        <>
            <section className="relative flex gap-10 py-0 m-0 max-md:flex-col">
                <div className="sticky top-[80px] z-30">
                    ...
                    <div className={"min-w-[200px] h-[calc(100vh-80px-60px)] md:h-cover md:sticky top-24 overflow-y-auto p-6 md:pr-0 md:border-grey md:border-r absolute max-md:top-[64px] bg-white max-md:w-[calc(100%+80px)] max-md:px-16 max-md:-ml-7 duration-500 " + (!showSideNav ? "max-md:opacity-0 max-md:pointer-events-none" : "opacity-100 pointer-events-auto") + "overflow-y-scroll" } style={{ scrollbarWidth: 'thin', scrollbarColor: 'transparent transparent' }}>
                        <h1 className="text-xl text-dark-grey mb-3">Dasbor</h1>
                        <hr className="border-grey -ml-6 mb-8 mr-6" />

                        {
                            isAdmin ?
                            <>
                                <NavLink to="/dashboard/manage-blogs" onClick={(e) => setPageState(e.target.innerText)} className="sidebar-link">
                                    <i className="fi fi-rr-document"></i>
                                    Artikel
                                </NavLink> 
                                
                                <NavLink to="/dashboard/manage-users" onClick={(e) => setPageState(e.target.innerText)} className="sidebar-link">
                                    <i className="fi fi-rr-users-alt"></i>
                                    Pengguna
                                </NavLink> 

                                <NavLink to="/dashboard/insights" onClick={(e) => setPageState(e.target.innerText)} className="sidebar-link">
...

                        {
                            isAdmin ? "" :
                                ...
                                <NavLink to="/dashboard/insight" onClick={(e) => setPageState(e.target.innerText)} className="sidebar-link">
                                    <i className="fi fi-rr-chart-histogram"></i>
                                    Statistik
                                </NavLink> 
